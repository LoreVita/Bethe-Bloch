...
float stopping_power_mean(float gam, float E0 ) //gam=fattore di Lorentz della particella ; E0=massa a riposo particella proiettile in MeV/c^2
{
...
{	
	...
	float SP_mean = stopping_power_mean(gam, E0);
	...
	for (int i=0; i<n; i++){
	    ...
	    E_mean = (Ei+Ef)/2;
	    gam = 1+E_mean/E0;
	    SP_mean = stopping_power_mean(gam, E0);
		dx = (Ei-Ef)*(1/SP_mean);
	    x = x+dx;
	...
	//Stampa del risultato
	cout << "Lo spessore necessario risulta essere pari a " << x << " cm" << endl;
